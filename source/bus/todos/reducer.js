...
export const todosReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case types.TOGGLE_COMPLETED_TODO:
            return state.map((item) => item.get("id") === action.payload.id ? item.update("completed", value => !item.get("completed")) : item);
        case types.ALL_COMPLETED_TODOS:
            return state.map((item) => item.update("completed", value => true));
        default:
            return state;
    }
};
